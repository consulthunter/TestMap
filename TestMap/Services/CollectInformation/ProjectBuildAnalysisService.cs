...
public class ProjectBuildAnalysisService(ProjectContext context) : IProjectBuildAnalysisService
{
    ...
    {
        if (IsTrue(GetEffectiveProperty(evaluatedProject, "IsTestProject")))
        {
            return true;
        }
        ...
        WindowsRequirementType windowsRequirement)
    {
        ...
        {
            parts.Add($"SDK {sdkVersion}");
        }

        if (!string.IsNullOrWhiteSpace(globalJsonPath))
        {
            parts.Add($"global.json: {globalJsonPath}");
        }

        if (buildTargets.Count > 0)
        {
            parts.Add($"targets: {string.Join(", ", buildTargets)}");
        }

...
public class TestGenerator
{
    ...
    public async Task<string> CreateTest(string prompt, double temperature = 0.0, CancellationToken cancellationToken = default)
    {
        ...
        var providerConfig = _config.AiProviderConfig.GetProviderConfig(generationConfig.Provider)
                            ?? throw new InvalidOperationException($"Provider config not found for {generationConfig.Provider}.");

        if (!_providers.TryGetValue(generationConfig.Provider, out var provider))
        {
            throw new InvalidOperationException($"Provider implementation not registered for {generationConfig.Provider}.");
        }

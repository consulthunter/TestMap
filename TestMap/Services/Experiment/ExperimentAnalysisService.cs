...
public class ExperimentAnalysisService : IExperimentAnalysisService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new ArgumentException($"Experiment run {experimentRunId} not found");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await (
                ...
                join testMemberLeft in _dbContext.Members on candidate.ExistingTestMemberId equals testMemberLeft.Id into testMembers
                from testMember in testMembers.DefaultIfEmpty()
                join testObjectLeft in _dbContext.Objects on testMember.ObjectEntityId equals testObjectLeft.Id into testObjects
                ...
                join testProjectLeft in _dbContext.CSharpProjects on testFile.CSharpProjectId equals testProjectLeft.Id into testProjects
    ...
    private async Task<double?> GetLatestTestDurationMsAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (_context.Project.DbId == 0 || (string.IsNullOrWhiteSpace(testName) && !memberId.HasValue))
        {
            return null;
        }
        ...
        if (memberId.HasValue)
        {
            ...
            if (resultByMemberId.HasValue)
            {
                return resultByMemberId;
            }
        }

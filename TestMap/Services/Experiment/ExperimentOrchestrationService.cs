...
public class ExperimentOrchestrationService : IExperimentOrchestrationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                GenerationStrategy.Pass1 => [await ExecutePass1Async(candidateMethod.Id, context, provider, cancellationToken)],
                GenerationStrategy.Pass5 => await ExecutePass5Async(candidateMethod.Id, context, provider, cancellationToken),
                GenerationStrategy.Repair5 => await ExecuteRepair5Async(candidateMethod.Id, context, provider, cancellationToken),
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var result = await _pipeline.GenerateTestAsync(CreateGenerationRequest(context, provider), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        TestExecution? lastExecution = null;
        var attempts = new List<GenerationAttempt>();
        ...
        for (int i = 1; i <= 5; i++)
        {
            ...
            try
            {
                ...
                if (i == 1)
                {
                    result = await _pipeline.GenerateTestAsync(CreateGenerationRequest(context, provider), cancellationToken);
                }
                else
                {
                    ...
                    {
                        MethodBody = context.Method.SourceCode,
                        MethodName = context.Method.MethodName,
                        GeneratedTest = currentTest,
                        ...
                        TestSupportContext = context.TestSupportContext,
                        ExampleTestMetadataSummary = context.ExampleTestMetadataSummary,
                        ProjectTestMetadataSummary = context.ProjectTestMetadataSummary,
                        CoverageGapSummary = context.CoverageGapSummary,
                        ErrorLogs = lastExecution.ErrorLogs ?? "Test failed",
                        StructuredErrors = lastExecution.StructuredErrors,
                        PriorConversationTranscript = currentConversationTranscript,
                        Provider = provider,
                        Temperature = 0.0,
                        AttemptNumber = i,
                        StepErrorRetries = Math.Max(0, _activeExperimentConfig?.StepErrorRetries ?? 0),
                        StepRetryDelayMs = Math.Max(0, _activeExperimentConfig?.StepRetryDelayMs ?? 1000),
    ...
    private async Task<TestExecution> ExecuteAndTestAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (!Utilities.Utilities.InsertTestIntoFile(
                    context.TestClassName,
                    context.TestFilePath,
                    generatedTest))
            {
                throw new InvalidOperationException(
                    $"Failed to insert generated test into {context.TestFilePath} for class {context.TestClassName}.");
            }
            ...
            execution.Classification = execution.TestPassed
                ...
                : (execution.CoverageImprovement > 0 ? TestClassification.Candidate : TestClassification.Failed);
            ...
            if (!execution.CompilationSuccess)
            {
                ...
                execution.FailureSummary = buildResult.FailureAnalysis?.Summary ?? "Build failed before test execution.";
            }
    ...
    private async Task RefreshProjectMetadataAsync(string testProjectPath)
    {
        ...
        {
            await _testSmellService.CollectAsync(testProjectPath, _context.Project.DbId);
        }
    }
    ...
    private List<AiProvider> GetProvidersToTest(ExperimentConfiguration config)
    {
        ...
        if (providers.Count == 0)
            throw new InvalidOperationException("No usable AI providers were found in AiProviderConfig. Configure at least one provider before running experiments.");
    ...
    private AiProvider ParseConfiguredProvider(string providerName)
    {
        ...
        if (providerConfig == null || !AiProviderConfigurationRules.IsUsable(providerConfig))
        {
            ...
            throw new InvalidOperationException($"Provider '{provider}' is not configured for experiment use. {detail}");
        }
    ...
    private GenerationAttempt CreateAttempt(int candidateMethodId, AiProvider provider, GenerationStrategy strategy, int attemptNumber)
    {
    ...
    private GenerationAttempt CreateFailedAttempt(int candidateMethodId, AiProvider provider, GenerationStrategy strategy, int attemptNumber, string errorMessage)
    {
        return new GenerationAttempt
        {
            CandidateMethodId = candidateMethodId,
    ...
    private static TestExecution CreateFailedExecution(TestFailureKind failureKind, string? errorMessage)
    {
    ...
    private static TestFailureKind MapFailureKind(Models.Results.FailureAnalysisModel failureAnalysis)
    {

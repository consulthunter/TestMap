...
def build_parser() -> argparse.ArgumentParser:
    ...
    targeted_stryker_parser = subparsers.add_parser(
        ...
        help="Run Stryker from one source project directory.",
    )
...
def run_dotnet_stryker_project(args: argparse.Namespace) -> int:
    ...
    source_project_path = resolve_stryker_source_project_path(args.project, test_project_path)
    if source_project_path is None:
        print(f"Source project not found in container: {args.project}")
        return 1

    source_project_dir = source_project_path.parent
    ...
    if args.target_framework:
        target_framework_args = ["--target-framework", args.target_framework]
...
def resolve_stryker_source_project_path(project_argument: str, test_project_path: Path) -> Path | None:
    project_path = Path(project_argument)
    candidate_path = project_path if project_path.is_absolute() else test_project_path.parent / project_path

    if candidate_path.exists() and not same_path(candidate_path, test_project_path):
        return candidate_path.resolve()

    referenced_project = find_first_project_reference(test_project_path)
...
def find_first_project_reference(test_project_path: Path) -> Path | None:
    try:
        root = ElementTree.parse(test_project_path).getroot()
    except (ElementTree.ParseError, OSError):
        return None

    for element in root.iter():
        if strip_xml_namespace(element.tag) != "ProjectReference":
            continue
        ...
        referenced_project = (test_project_path.parent / normalize_project_reference_include(include)).resolve()
        if referenced_project.exists() and not same_path(referenced_project, test_project_path):
            return referenced_project

    return None


def normalize_project_reference_include(include: str) -> str:
    return include.replace("\\", os.sep).replace("/", os.sep)
